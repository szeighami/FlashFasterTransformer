...
template<typename T>
void free_mem(int no_layers, T** qkv_buf, T** key_cache, T** value_cache, T** context_buf)
{
...
void allocate_mem(const KERNEL_PARAMS_TYPE& params, int no_layers, T*** qkv_buf, T*** key_cache, T*** value_cache, T*** context_buf)
{
...
template<typename T, int Dh, int Dh_MAX, typename KERNEL_PARAMS_TYPE>
bool forward(KERNEL_PARAMS_TYPE params, const cudaStream_t& stream, int no_layers, T** qkv_buf, T** key_cache, T** value_cache, T** context_buf){
    //settings for A100
...
void call_attention(int max_input_len, int max_tokens, int batch_size,int head_num,int no_layers){
    ...
    T** qkv_buf; T** key_cache; T** value_cache; T** context_buf;
    allocate_mem<T>(params, no_layers, &qkv_buf, &key_cache, &value_cache, &context_buf);
    cudaDeviceSynchronize();

    int reps=5; int warmup=3;

    for (int r = 0; r < warmup; r++)
        forward<T, size_per_head, size_per_head, Masked_multihead_attention_params<T>>(params, 0, no_layers, qkv_buf, key_cache, value_cache, context_buf);
    cudaDeviceSynchronize();
    ...
    printf("batch=%d,prompt_len=%d,max_tokens=%d,precision=%lu,size_per_head=%d,time%f\n", batch_size, max_input_len, max_tokens, sizeof(T)*8,size_per_head, ms_took/(float)reps);


    free_mem<T>(no_layers, qkv_buf, key_cache, value_cache, context_buf);
    ...
    switch (precision){
        case 16:
            switch (size_per_head){
                case 64:
                    call_attention<uint16_t, 64>(prompt_len, max_tokens, batch_size,head_num,no_layers);
                    break;
                case 128:
                    call_attention<uint16_t, 128>(prompt_len, max_tokens, batch_size,head_num,no_layers);
                    break;
                case 256:
                    call_attention<uint16_t, 256>(prompt_len, max_tokens, batch_size,head_num,no_layers);
                    break;
                default:
        ...
        case 32:
            switch (size_per_head){
                case 64:
                    call_attention<float, 64 >(prompt_len, max_tokens, batch_size,head_num,no_layers);
                    break;
                case 128:
                    call_attention<float, 128>(prompt_len, max_tokens, batch_size,head_num,no_layers);
                    break;
                case 256:
                    call_attention<float, 256>(prompt_len, max_tokens, batch_size,head_num,no_layers);
                    break;
                default:
                    printf("Not support head size\n");
                    break;
