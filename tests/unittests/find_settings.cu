...
int main(int argc, char* argv[]) {
   ...
   if (save_res){
       ...
       if (!exists){
           ...
           res_file << "batch,prompt_len,max_tokens,tpv,tpk,tpb,time\n";
           res_file.close();
       }
    }
            ...
            for (int max_input_len : max_input_lens){
                ...
                */
                ...
                for (int r = 0; r < warmup; r++)
                    forward<T, size_per_head, size_per_head, Masked_multihead_attention_params<T>>(params, 0, 0, 0, 0, no_layers, qkv_buf, key_cache, value_cache, context_buf, true);
                cudaDeviceSynchronize();
                ...
                printf("Default Took %f ms\n", ms_took/(float)reps);
                
                if (save_res){
                   res_file.open(path, std::ios::app);
                   res_file << batch_size<<"," << max_input_len<<","<<max_tokens<<",-1,-1,-1,"<<ms_took/(float)reps<<"\n";
                   res_file.close();
                }

                if (validate_res){
                    bool success = forward<T, size_per_head, size_per_head, Masked_multihead_attention_params<T>>(params, 0, 0, 0, 0, no_layers, qkv_buf, key_cache, value_cache, context_buf, true, true_res_host);
                    if (!success){
                ...
                for (int v = 0; v < sizeof(tpvs)/sizeof(tpvs[0]); v++){
                    for (int k = 0; k < sizeof(tpks)/sizeof(tpks[0]); k++){
                        for (int b = 0; b < sizeof(tpbs)/sizeof(tpbs[0]); b++){
                            ...
                            secs_used=(end.tv_sec - start.tv_sec); //avoid overflow by subtracting first
                            ms_took= (((secs_used*1000000.0) + end.tv_usec) - (start.tv_usec))/1000.0;
                            printf("batch=%d,prompt_len=%d,max_tokens=%d,tpv=%d,tpk=%d,tpb=%d,time%f\n", batch_size, max_input_len, max_tokens, tpvs[v], tpks[k], tpbs[b], ms_took/(float)reps);

                            if (save_res){
                               res_file.open(path, std::ios::app);
                               res_file << batch_size<<"," << max_input_len<<","<<max_tokens<<","<<tpvs[v]<<","<<tpks[k]<<","<<tpbs[b]<<","<<ms_took/(float)reps<<"\n";
                               res_file.close();
                            }
                        }
                    }
