...
struct mmha_lahnch_TPV<T, Dh, Dh_MAX, TPK, TPB, KERNEL_PARAMS_TYPE, 4>{
    static void call(const KERNEL_PARAMS_TYPE& params, const cudaStream_t& stream, int v_vec_size)
    {
        ...
        if (max_threads/params.num_heads*params.batch_size < 128) {
            MMHA_LAUNCH_KERNEL_OPTIMIZED(T, Dh, Dh_MAX, 4, THREADS_PER_VALUE, 64, DO_CROSS_ATTENTION, stream);
        }
        else if (max_threads/params.num_heads*params.batch_size < 256) {
            MMHA_LAUNCH_KERNEL_OPTIMIZED(T, Dh, Dh_MAX, 2, THREADS_PER_VALUE, 128, DO_CROSS_ATTENTION, stream);
        }
        else {
